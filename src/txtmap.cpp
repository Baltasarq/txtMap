		...
		: scanSCE(sc), fout(NULL), log(NULL), modoLimpio(cl)
{
        ...
        // con esto el fichero ser�a fich.ext.log
...
void ParserAvNat::procesaInventario(Localidad *loc)
{
        ...
        ponLogStr( "A�adiendo objetos al inventario." );
...
void ParserAvNat::procesaObjetoEscenario(Localidad *loc)
{
        ...
        // Tomar las sigs l�neas (descripci�n)
...
void ParserAvNat::procEntrada(void)
{
        ...
        // Tomar primera l�nea de la entrada con info
        linact = scanSCE->leeLinea();
...
void Localidad::guardar(FILE *f)
{
        ...
        // Guardar recursos gr�fico-sonoros
...
// Eliminar los guiones precedentes en argumentos de l�nea de comando.
{
        size_t guion = 0;
	...
	while ( argAct < argc )
	{
		...
		if ( arg[0] == '-' )
		{
			...
			&& arg.compare(0, opMkdir.length(), opMkdir) == 0)
            {
				...
				mostrarSintaxis();
				throw ErrorSintaxis( "opci�n desconocida '" +
            ...
            porPantalla( "\nf  : " );
                    ...
                    || !Persistente::leerCabecera( fichEnt->getDescriptor() ) )
                  {
                        ...
                        if (  sce == NULL
                         ||!( sce->preparado() ) )
                        {
                              porPantalla("ERROR El archivo no existe o no est� "
                                            "disponible.\n");
                        }
                        else {
