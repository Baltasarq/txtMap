...
const std::string StringMan::CaracteresEspecialesMins = "��������";
const std::string StringMan::CaracteresEspecialesMays = "��������";
...
std::string &StringMan::normCnvt(std::string &x)
{
	...
	// Quitar los acentos y pasar a may�sculas
        maysCnvt( x, EliminaAcentos );

	// Quitar cualquier car�cter no imprimible
...
std::string &StringMan::cambiarFmtImprCnvt(std::string &x)
{
        ...
        // Poner la primera en may�sculas
        x[ 0 ] = maysCnvtCh( x[ 0 ] );

        // Poner las secundarias en may�sculas (despu�s de espacios)
        pos = x.find(' ');
        ...
        // Poner las secundarias en may�sculas (despu�s de guiones)
        pos = x.find( '-' );
...
bool StringMan::esNumeroDecimal(const std::string &strNum)
{
		...
		// S�lo puede haber un punto, se tiene que haber revisado toda la cadena
		if ( numPuntos <= 1
		  && pos == strNum.end() )
		{
			toret = true;
...
/*
	Es necesario recalcular los punteros, ya que al cambiar el tama�o
	de la cadena, es posible que �sta ya no est� en la antigua posici�n
	en memoria.
*/
{
